$body_id = "HomePage";
<?php include 'header.inc'; ?>
        
        <main>

            <section>
                <h1>Looking for Cybersecurity Specialists</h1>
                <p>
                    <strong>One Studio</strong> is looking for passionate and experienced cybersecurity professionals
                    from all around the world. Our goal is to give everyone a fair chance, no matter their age, gender,
                    or nationality.
                </p>
                <p>
                    One Studio currently has three available roles:
                    <strong>Ethical Hacking</strong>, <strong>Security Analysis</strong>, and
                    <strong>Threat Hunting</strong>.
                </p>
            </section>

            <section>
                <h2>Brief Description of Our Cybersecurity Roles</h2>

                <h3>Ethical Hacking</h3>
                <ul>
                    <li>Test computer systems, networks, and applications.</li>
                    <li>Identify vulnerabilities before attackers can exploit them.</li>
                    <li>Document findings and help fix discovered weaknesses.</li>
                </ul>

                <h3>Security Analysis</h3>
                <ul>
                    <li>Monitor endpoints and user behaviour data for suspicious activity.</li>
                    <li>Investigate alerts and incidents from security tools.</li>
                    <li>Assist in developing and improving security policies.</li>
                </ul>

                <h3>Threat Hunting</h3>
                <ul>
                    <li>Proactively search for hidden threats in networks and endpoints.</li>
                    <li>Use threat intelligence and behaviour analysis to detect attacks.</li>
                    <li>Work with the wider security team to improve defences over time.</li>
                </ul>
            </section>

            <section>
                <h2>Start Your Journey right now with One Studio</h2>
                <p>
                    If you think you have what it takes, visit our Apply page to submit your application. If you want
                    more information about each role, review the details on our Job Opportunities page.
                <div class="cta-cards">
                    <div class="cta-card">
                        <a href="apply.php">Apply Now</a>
                     </div>

                    <div class="cta-card">
                        <a href="jobs.php">View Job Opportunities</a>
                    </div>
                </div>

            </section>
        </main>

<?php include 'footer.inc'; ?>